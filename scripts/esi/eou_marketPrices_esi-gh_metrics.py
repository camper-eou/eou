...
def analyze_buy(entries: List[Tuple[float, int]]) -> Dict:
    ...
    clean_entries = [
        (float(p), int(u))
        for p, u in entries
        if int(u) > 0 and float(p) not in highliner_prices and float(p) not in lowliner_prices
    ]
...
def analyze_sell(entries: List[Tuple[float, int]]) -> Dict:
    ...
    clean_entries = [
        (float(p), int(u))
        for p, u in entries
        if int(u) > 0 and float(p) not in lowliner_prices and float(p) not in highliner_prices
    ]
    clean_vwap = _vwap(clean_entries)
